...
# --------------------------------------------------------
def get_3d_sincos_pos_embed(embed_dim, w,h,d, cls_token=False, flag=False):
    """
    grid_size: int of the grid height and width
    return:
    pos_embed: [grid_size*grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    """
    grid_h = np.arange(w, dtype=np.float32)
    grid_w = np.arange(h, dtype=np.float32)
    grid_d = np.arange(d, dtype=np.float32)
    grid = np.meshgrid(grid_d, grid_w, grid_h)  # here w goes first
    grid = np.stack(grid, axis=0)

    grid = grid.reshape([3, 1, w, h, d])
