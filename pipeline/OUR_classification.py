...
import model.OUR_resnet3D as GM_NEW_resnet3D
import module.NEW_data_set_pathological_classification as NEW_data_set
...
for fold in range(0, 10):
    ...
    # 设置模型
    model = GM_NEW_resnet3D.resnet50().to(device)
    # 设置激活函数
    ...
    ALL_pretrain_model = gen_models.mae_vit_base_patch16_dec512d8b().to(device)
    # White_pretrain_model = gen_models.mae_vit_base_patch16_dec512d8b().to(device)
    # Gray_pretrain_model.load_state_dict(torch.load('weights/Gray_mae3d_gan_wegihts_11.pth'))
    ALL_pretrain_model.load_state_dict(torch.load('weights/White_mae3d_gan_wegihts_13_10.pth'))
    # White_pretrain_model.load_state_dict(torch.load('weights/White_mae3d_gan_wegihts_11.pth'))
    ...
    for i in range(epoch):
        ...
        for data in train_loader:
            ...
            gray_data_1_2 = ALL_pretrain_model(gray_data_1_1)
...
#             loss1 = loss_fn(outputs1, target1)
            ...
            if total_loss1 <= min_train_loss:
                ...
                torch.save(model.state_dict(),
                           "result_model/OUR_result_model/CN_vs_AD_our_GW_vit_13_finaltest_minloss_model_{}.pth".format(fold + 1))
...
#
    # 10折验证测试
    model = GM_NEW_resnet3D.resnet50()
    model.load_state_dict(
        torch.load('result_model/OUR_result_model/CN_vs_AD_our_GW_vit_13_finaltest_minloss_model_{}.pth'.format(fold + 1)))
    ...
    # Gray_pretrain_model = gen_models.mae_vit_base_patch16_dec512d8b().to(device)
    # White_pretrain_model = gen_models.mae_vit_base_patch16_dec512d8b().to(device)
    # Gray_pretrain_model.load_state_dict(torch.load('weights/White_mae3d_gan_wegihts_13_75.pth'))
    # White_pretrain_model.load_state_dict(torch.load('weights/White_mae3d_gan_wegihts_12_29.pth'))
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            gray_data_3_2 = ALL_pretrain_model(gray_data_3_1)
            ...
            df_label.to_csv('result_csv/OUR_result/CN_vs_AD_our_GW_vit_12_29_finaltest_label_{}.csv'.format(fold + 1), encoding='gbk',
                            index=False)
            label_name = ["CN", "AD"]
            df_pred = pd.DataFrame(data=pred_list, columns=label_name)
            df_pred.to_csv('result_csv/OUR_result/CN_vs_AD_our_GW_vit_12_29_finaltest_pred_result_{}.csv'.format(fold + 1),
...
acc_list.to_csv('result_csv/OUR_result/CN_vs_AD_our_GW_vit_12_29_finall_acc_all.csv', encoding='gbk', index=False)
# 保存每次验证的准确率
csv_name_val_acc = ['acc']
acc_list = pd.DataFrame(data=val_max_acc, columns=csv_name_val_acc)
acc_list.to_csv('result_csv/OUR_result/CN_vs_AD_our_GW_vit_12_29_val_acc_all.csv', encoding='gbk', index=False)
