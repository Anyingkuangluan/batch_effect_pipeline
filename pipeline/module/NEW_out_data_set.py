...
def robust_scale_3d(array1, array2):
    # 计算中位数和四分位距
    median = np.median(array1, axis=(1, 2), keepdims=True)
    q1 = np.percentile(array1, 25, axis=(1, 2), keepdims=True)
    q3 = np.percentile(array1, 75, axis=(1, 2), keepdims=True)
    iqr = q3 - q1

    # 使用中位数和四分位距进行缩放
    scaled_array = (array2 - median) / iqr
    return scaled_array
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):  # 检索函数
        ...
        label = 0
        if self.label == "out_NC":
            label = 0
        if self.label == "out_AD":
            label = 1
